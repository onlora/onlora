...
interface RawFeedItem {
  ...
  score?: number | null
  author?: RawFeedItemAuthorNested | null // For /latest structure
...
/**
 * Fetches a specific feed type (latest, trending, following).
 * @param feedType Type of feed to fetch.
 * @param page Page number (1-indexed).
 * @param pageSize Number of items per page.
 * @param options Optional fetch options (e.g., credentials for /following).
 */
async function fetchFeed(
  feedType: 'latest' | 'trending' | 'following',
  page = 1,
  pageSize = 20,
  ...
  const normalizedData = rawData.data.map((item: RawFeedItem): FeedPost => {
    ...
    if (item.author) {
      ...
      author = {
        ...
        name: item.author_username ?? null,
